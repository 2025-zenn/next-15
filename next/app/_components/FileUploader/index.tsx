...
const FileUploader = ({ onClose }: FileUploaderProps) => {
  ...
  const [healthAdvice, setHealthAdvice] = useState<HealthAdviceResult | null>(
    null
  );

  const addDataToFirestore = async (response: string) => {
    try {
      const foodsData = JSON.parse(response);
      const docRef = await addDoc(collection(db, "uploadData"), foodsData);
                      ...
                      <div className="border-b pb-3">
                        <p className="text-sm font-medium text-gray-700">
                          ...
                          <span className="text-blue-600">
                            {result["min-total-calories"]}~
                            {result["max-total-calories"]}kcal
                          </span>
                        </p>
                      </div>
                      <div>
                          ...
                          {result.foods.map((food: FoodItem, index: number) => (
                            ...
                            >
                              ...
                              <span className="text-sm text-gray-600 ml-2">
                                {food["min-calories"]}~{food["max-calories"]}
                                kcal
                              </span>
                            </li>
                          ))}
      ...
      {!analysisResult && (
        <>
          <div className="flex items-center justify-center w-full">
            <label
              htmlFor="file-upload"
              className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600"
            >
